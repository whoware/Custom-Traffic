...
local function create_trolly_vehicle(targetId, vehicleHash, pedHash)
	...
	local distanceXY_value = 100
	local distanceZ_value = 3
	local jumpHeight_value = 8
	local dynamic_force_multiplier_slider = createMenuSlider(NPCJumpCore, "Dynamic Force Multiplier", "Sensitivity of the force applied to NPCs based on player speed.", 0, 100, dynamic_force_multiplier_value, 1, function(value) dynamic_force_multiplier_value = value end)
	local constant_multiplier_slider = createMenuSlider(NPCJumpCore, "Constant Multiplier", "Constant value added to dynamic force calculation.", 0, 200, constant_multiplier_value, 1, function(value) constant_multiplier_value = value end)
	local player_speed_divisor_slider = createMenuSlider(NPCJumpCore, "Player Speed Divisor", "Value that the player's speed is divided by in dynamic force calculation.", 1, 100, player_speed_divisor_value, 1, function(value) player_speed_divisor_value = value end)
	local prediction_time_slider = createMenuSlider(NPCJumpCore, "Prediction Time", "Time used to predict player position in future. (x0.1 second)", 10, 100, prediction_time_value, 10, function(value) prediction_time_value = value end) -- Set step as 10
	local max_height_difference_slider = createMenuSlider(NPCJumpCore, "Max Height Difference", "Maximum difference in Z axis between NPC and player for jump.", 1, 10, max_height_difference_value, 1, function(value) max_height_difference_value = value end)
	local distanceXY_slider = createMenuSlider(NPCJumpCore, "Max XY Distance", "Maximum 2D distance between NPC and player for jump.", 10, 200, distanceXY_value, 1, function(value) distanceXY_value = value end)
	local distanceZ_slider = createMenuSlider(NPCJumpCore, "Min Z Distance", "Minimum Z distance between NPC and player for jump.", 1, 10, distanceZ_value, 1, function(value) distanceZ_value = value end)
	local jumpHeight_slider = createMenuSlider(NPCJumpCore, "Jump Height", "Force applied to NPC's Z axis when jumping.", 1, 20, jumpHeight_value, 1, function(value) jumpHeight_value = value end)
...
local worldOptions <const> = menu.list(menu.my_root(), translate("World", "World"), {}, "THESE FUNCTIONS ARE NOT OFFICIALLY SUPPORTED BY WHOWARE, PLEASE GO TO > Players TO USE CUSTOM TRAFFIC")
...
local services <const> = menu.list(menu.my_root(), translate("Services", "Services"), {}, "THESE FUNCTIONS ARE NOT OFFICIALLY SUPPORTED BY WHOWARE, PLEASE GO TO > Players TO USE CUSTOM TRAFFIC")
...
local script <const> = menu.list(menu.my_root(), "WiriScript", {}, "THESE FUNCTIONS ARE NOT OFFICIALLY SUPPORTED BY WHOWARE, PLEASE GO TO > Players TO USE CUSTOM TRAFFIC")
