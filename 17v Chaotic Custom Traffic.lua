--==||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||==--
--==||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||     v 17   WHOWARE CUSTOM TRAFFIC & POWERS   v 17      ||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||==--
--==||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||     v 17   WHOWARE CUSTOM TRAFFIC & POWERS   v 17      ||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||==--
--==||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||==--
...
--==||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||==--
	...
	local max_health_slider = createMenuSlider(CTLCustoms, "Max Health", "", 0, 100000, 6000, 500, function(value) 
		max_health = value
		if isDebugOn then
			util.toast("max health applied")
		end
	end)
	
	local health_slider = createMenuSlider(CTLCustoms, "Health", "", 0, 100000, 6000, 500, function(value) 
		health = value
	end)
	...
	local proximity_push_threshold = 15
	local proximity_push_force_multiplier = 1
	local proximity_distance_threshold = 1
		...
		local function customTrafficLoop()
			while true do
						...
						if is_player_active(pId, false, true) and isCustomTrafficOn then
							...
							for _, vehicle in ipairs(get_vehicles_in_player_range(pId, 20.0)) do
								if playerVehicle ~= 0 and vehicle ~= playerVehicle then
									...
									if distance_to_player <= push_threshold and current_ms() - push_timer >= push_cooldown then
										...
										ENTITY.APPLY_FORCE_TO_ENTITY(playerVehicle, 1, force.x, force.y, force.z, 0, 0, 0, true, false, true, false, true)
										push_timer = current_ms()
									end
								end
							end
						end
				...
				if settings2.audioSpam.isOn then --AP2
					if isDebugOn then
						util.toast("audioSpam2 detected as activated for some reason.")
					end
					for pId = 0, 31 do
						if is_player_active(pId, false, true) and isCustomTrafficOn then
							local targetPed = PLAYER.GET_PLAYER_PED_SCRIPT_INDEX(pId)
