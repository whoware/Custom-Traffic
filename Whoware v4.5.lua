...
-- add condition that makes it so npcs can only jump if they are looking at the player
	...
	local GlobalTextVariables = {
		...
		npcJumpActivatedxy = {
			TextColor = {r = 1, g = 0.7, b = 0.7, a = 1}, 
			x = 0.50, 
			y = 0.88  
		},
		playerProximityHomingActivatedxy = {
			TextColor = {r = 1, g = 0.8, b = 0.7, a = 1}, 
			x = 0.50, 
			y = 0.90  
		},
	...
	local downwards_force = -1.395 * gravity_multiplier
		...
		local function customTraffic(pId)
			...
			function drawBruteForceActivated()
				if os.clock() - bruteForceTime < 0.6 then -- if less than 3 seconds has passed
			...
			function drawPlayerBruteForceActivated()
				if os.clock() - playerbruteForceTime < 0.6 then -- if less than 3 seconds has passed
			...
			function drawProximityHomingActivated()
				if os.clock() - proximityHomingTime < 0.8 then -- if less than 3 seconds has passed
			...
			function drawPlayerProximityHomingActivated()
				if os.clock() - playerProximityHomingTime < 0.7 then -- if less than 3 seconds has passed
			...
			function drawNPCJumpActivated()
				if os.clock() - npcJumpTime < 0.5 then -- if less than 3 seconds has passed
			...
			function drawExplodeOnCollisionActivated()
				if os.clock() - explodeOnCollisionTime < 0.5 then -- if less than 3 seconds has passed
