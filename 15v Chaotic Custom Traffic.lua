...
ServerSide = function(pId)
    ...
    if not menu_already_added then -- check if we haven't already added these options
        ...
        menu_already_added = true -- set the flag so we don't add these options again
    end
...
--==||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||==--
	...
	local CTLCustoms = menu.list(ChaoticOpt, translate("Trolling", "CTL Customs • SUPER | POWERS •         >"), {}, "Modify RAW Custom Traffic Functions in REALTIME, that of which all control the behavior of NPCs and the script while | Custom Traffic • NORMAL | MODE • RAW | is enabled.")
	...
	local playerProximity_push_threshold = 30
	...
	local function drawPreset(presetName, presetData)
		...
		local function adjust_npc_trajectory(npc, targetPosition)
			...
			local desiredSpeed = math.max(1, verticalDistance / 2) -- adjust this formula based on your requirements
			...
			-- Calculate the angle between the NPC's current position and the target position
			local angle = math.deg(math.atan2(dy, dx))
			-- Set the NPC's rotation to this angle
			ENTITY.SET_ENTITY_ROTATION(npc, 0, 0, angle)
		end
		
		local function should_npc_jump(driver, player)
			if isDebugOn then
				util.toast("NPCJump is currently scanning for ideal jump condition opportunities. (i.e - if the players feet are at a slightly higher elevation compared to a given cars total height)")
			end
			...
			if distanceXY < 100 and distanceZ > 3 and driverCoords.z - playerCoords.z < max_height_difference then 
				...
				local speedMultiplier = 1 + vehicleSpeed / 50 -- Adjust this value based on your requirements
				local force = {
		...
		local function applyPlayerGravityControl(targetPed)
			if isDebugOn then
				util.toast("applyPlayerGravityControl was called.")
			end
			local playerVehicle = PED.GET_VEHICLE_PED_IS_IN(targetPed, false)
...
local function customTraffic(pId)
	...
	for _, vehicle in ipairs(get_vehicles_in_player_range(pId, npcRange)) do
		...
		local function updateTargetPlayers()
			if isDebugOn then
				util.toast("deleteDestroyedVehicles was called or is currently active. The heart of the script.")
			end
		...
		local function customTrafficLoop()
			...
			while true do
				...
				if isExplodeOnCollisionOn then --EoC
					for pId = 0, 31 do
						if is_player_active(pId, false, true) and isCustomTrafficOn then
							local targetPed = PLAYER.GET_PLAYER_PED_SCRIPT_INDEX(pId)
				...
				if isPlayerBruteForceOn then --pbf
					for pId = 0, 31 do
						if is_player_active(pId, false, true) and isCustomTrafficOn then
				...
				if settings.AnnoyingExplosiveNPCsOn.isOn then
					...
					AudioSpam_isOn = true
					isBruteForceOn = true
					push_force_multiplier = math.random(0, 3)
					proximity_distance_threshold = 6
					jumpHeight = 15
					downwards_force = -20
					jump_cooldown_value = 90
					proximity_push_force_multiplier = 1
					speedMultiplier = 9000
					npcRange = 50
					maxNpcDrivers = 7
				...
				if currentTime - lastCheckTime >= 1 then
					if callCount > 17 then
						...
						local sleepDuration = math.min(12, extraCalls * 2) -- Each extra call adds 50ms to sleep, capped at 1500ms
						util.yield(sleepDuration)
					else
						util.yield(0)  -- No sleep if calls are below or equal to 30
					end
					callCount = 0  -- Reset call counter for next second
					lastCheckTime = currentTime
				else
					util.yield(0)  -- No sleep if within the same second
				end
			end
		end
		...
		local function customTrafficToggle(on)
			...
			if not isCustomTrafficOn and isDebugOn then
...
--==||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||==--
		...
		local CustomTrafficLoop = menu.toggle(ChaoticOpt, translate("Ultimate Custom Traffic", "Custom Traffic • NORMAL | MODE • RAW > "), {}, false, customTrafficToggle)
		...
		local SchitzoToggle = menu.toggle(ChaoticOpt, "Schizophrenic Pedestrians", {}, false, function(on)
...
--==||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||==--
		...
		local NPCPowers = menu.list(superPowersMenu, translate("Ultimate Custom Traffic", "NPC Powers •     >"), {}, "Powers within this tab will apply to NPCDrivers and Chaotic Drivers.")
		...
		local AudioSpam1Components = {}
		AudioSpam1Components.AudioSpam = menu.list(customizeNPCPowers, translate("Customize Audio Spam •     >", "Customize Audio Spam •     >"))
		
		AudioSpam1Components.AudioSpamToggle = createMenuToggle(NPCPowers, "AP Audio Spam ", false, function(on) settings.audioSpam.isOn = on end)
		AudioSpam1Components.audioInvisibleToggle = createMenuToggle(AudioSpam1Components.AudioSpam, "AP Invisible Explosion @", true, function(on) settings.audioSpam.audioInvisible = on end)
		AudioSpam1Components.spamAudibleToggle = createMenuToggle(AudioSpam1Components.AudioSpam, "AP Audible @", true, function(on) settings.audioSpam.spamAudible = on end)
		AudioSpam1Components.TriggerDistanceSlider = createMenuSlider(AudioSpam1Components.AudioSpam, "AP Trigger Distance • >", "How far from the car to the player will count as a collision?", 1, 1000, 450, 25, function(value) settings.audioSpam.TriggerDistance = value / 10 end)
		AudioSpam1Components.audioTypeSlider = createMenuSlider(AudioSpam1Components.AudioSpam, "AP Audio Type • >", "Default: Predator Calls", 0, 82, 21, 1, function(value) settings.audioSpam.audioType = value end)
		AudioSpam1Components.audioDamageSlider = createMenuSlider(AudioSpam1Components.AudioSpam, "AP Damage • >", "Go nuts", 0, 1000, 0, 1, function(value) settings.audioSpam.audioDamage = value end)

		local audioSpamComponents2 = {}

		audioSpamComponents2.AudioSpam2 = menu.list(customizeNPCPowers, translate("Customize Audio Spam •     >", "Customize Audio Spam 2 •     >"))
		audioSpamComponents2.AudioSpamToggle2 = createMenuToggle(NPCPowers, "AP Audio Spam 2", false, function(on) settings2.audioSpam.isOn = on end)
		audioSpamComponents2.audioInvisibleToggle2 = createMenuToggle(audioSpamComponents2.AudioSpam2, "AP Invisible Explosion @", true, function(on) settings2.audioSpam.audioInvisible = on end)
		audioSpamComponents2.spamAudibleToggle2 = createMenuToggle(audioSpamComponents2.AudioSpam2, "AP Audible @", true, function(on) settings2.audioSpam.spamAudible = on end)
		audioSpamComponents2.TriggerDistanceSlider2 = createMenuSlider(audioSpamComponents2.AudioSpam2, "AP Trigger Distance • >", "How far from the car to the player will count as a collision?", 1, 1000, 450, 25, function(value) settings2.audioSpam.TriggerDistance = value / 10 end)
		audioSpamComponents2.audioTypeSlider2 = createMenuSlider(audioSpamComponents2.AudioSpam2, "AP Audio Type • >", "Default: Earthquake", 0, 82, 58, 1, function(value) settings2.audioSpam.audioType = value end)
		audioSpamComponents2.audioDamageSlider2 = createMenuSlider(audioSpamComponents2.AudioSpam2, "AP Damage • >", "Go nuts", 0, 1000, 0, 1, function(value) settings2.audioSpam.audioDamage = value end)
		...
		local superSpeedSlider = createMenuSlider(customizeNPCPowers, "Super Speed Multiplier • >", "Modify the engine power and wheel torque of NPC & Chaotic Drivers!", 0, 50000, 5000, 1000, function(value) speedMultiplier = value end)
		...
		local explodeTypeSlider = createMenuSlider(ExplodeOnCollisionMenu, "EoC Explosion Type • >", "P.S 59 is basically a nuke, 70 is EMP, 65 is Kenetic Mine...", 0, 82, explodeOnCollision_explosionType, 1, function(value) explodeOnCollision_explosionType = value end)
