...
local function create_trolly_vehicle(targetId, vehicleHash, pedHash)
	...
	local customTrollingOpt = menu.list(trollingOpt, translate("Trolling", "Ultimate Custom Traffic"), {}, "I Hope you enjoy Whoware Custom Traffic! Don't forget that you only need to turn a single 'Toggle Custom Traffic' on per lobby, it's recommended you customize your super powers and turn CTL onto yourself, then spectate people to affect them and anyone within the NPC attraction range from who you're spectating! CTL(Custom traffic loop) works by being enabled onto a single player. Once enabled, the mod will use whatever super power configuration across the ENTIRE lobby based on who you're spectating / whoever is near the person who has CTL enabled onto them. Have fun!")
	local isCustomTrafficOn = false
	local targetPed = PLAYER.GET_PLAYER_PED_SCRIPT_INDEX(pId)
	local selectedPlayerId = {}
	local npcDrivers = {}
	local npcDriversCount = 4
	local maxNpcDrivers = 4
	local npcRange = 50
	...
	--Explosion variables for "Explode on Collision"
	local isExplodeOnCollisionOn = false
	local explosionDistance = 3.350
	local explosionType = 66
	local explosionDamage = 1             
	local explosionInvisible = false
	local explosionAudible = false
	...
	local downwards_force = -1.395 * gravity_multiplier
		...
		local function should_npc_jump(driver, player)
			...
			local distanceXY = distance_2d(driverCoords, playerCoords)
			local distanceZ = math.abs(driverCoords.z - playerCoords.z)
			
			return distanceXY < 15 and distanceZ > 6
		end
		...
		local function realityWarp(pId, targetPed)
			for _, vehicle in ipairs(get_vehicles_in_player_range(pId, 100.0)) do
				local driver = VEHICLE.GET_PED_IN_VEHICLE_SEAT(vehicle, -1, false)
				if ENTITY.DOES_ENTITY_EXIST(driver) and not PED.IS_PED_A_PLAYER(driver) then
					local vehicleCoords = ENTITY.GET_ENTITY_COORDS(vehicle, true)
					local targetPedCoords = ENTITY.GET_ENTITY_COORDS(targetPed, true)
					...
					if distance_to_player <= settings.realityWarp.TriggerDistance then
						local warpSeed = math.random() -- random number for defining warp effect
						if warpSeed < 0.33 then -- sudden stop
							ENTITY.SET_ENTITY_VELOCITY(vehicle, 0, 0, 0)
						elseif warpSeed < 0.66 then -- sudden speed burst
							ENTITY.SET_ENTITY_VELOCITY(vehicle, vehicleCoords.x * settings.realityWarp.SpeedBurstMultiplier, vehicleCoords.y * settings.realityWarp.SpeedBurstMultiplier, vehicleCoords.z * settings.realityWarp.SpeedBurstMultiplier)
						else -- sudden lift off
							ENTITY.SET_ENTITY_VELOCITY(vehicle, vehicleCoords.x, vehicleCoords.y, vehicleCoords.z + settings.realityWarp.LiftOffHeight)
						end
					end
				end
			end
		...
		local function handleDestroyedNpcDriver(driver)
			if ENTITY.DOES_ENTITY_EXIST(driver) then
				...
				if playerVehicle ~= 0 and not VEHICLE.IS_VEHICLE_SEAT_FREE(playerVehicle, -1) then
					TASK.TASK_ENTER_VEHICLE(driver, playerVehicle, -1, -1, 1.0, 1, 0)
				end
			end
		end
		...
		local function customTraffic(pId) --ctl begin
			...
			for _, vehicle in ipairs(get_vehicles_in_player_range(pId, npcRange)) do
				...
				if TASK.GET_ACTIVE_VEHICLE_MISSION_TYPE(vehicle) ~= 6 then
					...
					if ENTITY.DOES_ENTITY_EXIST(driver) and not PED.IS_PED_A_PLAYER(driver) and npcDrivers[driver] == nil then
						...
						PED.SET_PED_MAX_HEALTH(driver, 600)
						ENTITY.SET_ENTITY_HEALTH(driver, 600, 0)
						PED.SET_BLOCKING_OF_NON_TEMPORARY_EVENTS(driver, true)
					end
				end
			end
		
			-- Call handleVehicleDestroyed function for each NPC driver
			for driver, vehicle in pairs(npcDrivers) do
		...
		local function removeExcessNPCDrivers() --its because everyone have 10
		...
		local function customTrafficLoop()
			...
			while true do
				...
				if settings.realityWarp.isOn then
					for pId = 0, 31 do
						if is_player_active(pId, false, true) and isCustomTrafficOn then
							local targetPed = PLAYER.GET_PLAYER_PED_SCRIPT_INDEX(pId)
							realityWarp(pId, targetPed)
						end
					end
				end
	
				if lastJump.elapsed() > 310 and isCustomTrafficOn then
					for pId = 0, 31 do
						if is_player_active(pId, false, true) then
							local targetPed = PLAYER.GET_PLAYER_PED_SCRIPT_INDEX(pId)
							...
							if is_player_in_tough_spot(pId) then
								for _, vehicle in ipairs(get_vehicles_in_player_range(pId, 150)) do
									...
									if not PED.IS_PED_A_PLAYER(driver) and should_npc_jump(driver, targetPed) then
										request_control_once(vehicle)
										ENTITY.APPLY_FORCE_TO_ENTITY(vehicle, 1, 0.0, 0.0, 13.0, 0.0, 0.0, 0.0, 0, false, false, true, false, false)
									end
								end
							end
						end
				...
				if isCustomTrafficOn then
					...
					for driver, vehicle in pairs(npcDrivers) do
						...
						if VEHICLE.IS_VEHICLE_DRIVEABLE(vehicle, false) == false or ENTITY.IS_ENTITY_DEAD(vehicle) then
							...
							handleDestroyedNpcDriver(driver)  --handle the destroyed NPC driver
						end
					end
					removeExcessNPCDrivers()
				else
		...
		local toggleLoop = menu.toggle(customTrollingOpt, translate("Ultimate Custom Traffic", "Toggle Custom Traffic @"), {}, false, customTrafficToggle)
		local despawnEntitiesToggle = createMenuToggle(customTrollingOpt, "Despawn Dead Peds and Empty Vehicles", false, function(on) settings.despawnEntities.isOn = on end)
		local npcDriversSlider = menu.slider(customTrollingOpt, translate("Ultimate Custom Traffic", "Number of NPC Drivers • >"), {}, "Control the amount of NPC Drivers you want chasing the player, decreasing the number deletes NPCs based on the value to save FPS",
		0, 126, 4, 1,
		function(value)
			maxNpcDrivers = value
			removeExcessNPCDrivers()
		end)
		local npcRangeSlider = menu.slider(customTrollingOpt, translate("Ultimate Custom Traffic", "NPC Attraction Range • >"), {}, "Set your own custom range for how far from the player NPCs will become reaped for control, go wild.",
		20, 460, 50, 10,
		function(value)
			npcRange = value
		end)
		local superPowersMenu = menu.list(customTrollingOpt, translate("Ultimate Custom Traffic", "Add SuperPowers • >"), {}, "Lets step it up a notch. Please note: Super powers and CTL need to only be applied to a single player per lobby! It is recommended you apply both super powers and CTL onto only yourself / or 1 other person!")


		local trollyVehicles <const> = menu.list(customTrollingOpt, translate("Ultimate Custom Traffic", "Trolly Vehicles"), {}, "Spawned Trolly Vehicles are considered 'controlled / reaped', therefor when spawned, they automatically aquire enabled super powers, whether Custom Traffic is enabled or not")
		local options <const> = {"Sandking", "Chimera", "Veto", "Skylift","Blazer", "rcbandito", "Cutter", "Thruster", "Phantom2", "Go-Kart", "AMBULANCE", "Chernobog", "Barracks", "Barrage", "Vetir", "Mule", "Pounder", "Biff", "Benson", "Cerberus", "Wastelander", "Forklift", "Docktug", "Mower", "Ripley", "Zhaba"}
		...
		menu.action(trollyVehicles, translate("Trolling - Trolly Vehicles", "Delete Trolly's"), {}, "Forces all functions except deletion from occuring, then spams the session with 10 delete requests within 3 seconds, after which the script releases your client from the intentional freeze. This is done in order to reduce lobby desync issues overtime due to entity overflowing via improper deletion", function()
			...
			menu.action(customTrollingOpt, translate("Trolling - Trolly Vehicles", "Delete NPCDrivers)"), {}, "Forces all functions except deletion from occurring, then spams the session with 10 delete requests within 3 seconds, after which the script releases your client from the intentional freeze. This is done in order to reduce lobby desync issues overtime due to entity overflowing via improper deletion", function()
				local start = os.clock()
				local interval = 3 / 10 -- executes 10 times within 3 seconds, so every 0.3 seconds
				local clearedEntities = 0 -- counter for cleared entities
	...
	function getClosestPlayer(currentPed)
		...
		local extrasList = menu.list(customTrollingOpt, translate("Ultimate Custom Traffic", "X Extra Traffic Options •     >"), {}, "Extra options / Boss mode Traffics / Beta options / FEATURES INSIDE ARE FOR FUN ONLY! = Might break script, if so, reload the script. Please note that these traffic options run OUTSIDE of 'Custom Traffic', super powers may still apply however the main loop wont, and as a result the auto clearing will not work since it's operating outside the loop, be warned.")
		...
		local playerGravityControlMenu = menu.list(customizePlayerPower, translate("Customize Player Gravity Control • >", "Customize Player Gravity Control • >"))
		local gravityMultiplierSlider = createMenuSlider(playerGravityControlMenu, "PGC Gravity Multiplier", "How strong is the gravity?", 0, 100, settings.playerGravityControl.gravity_multiplier, 1, function(value) settings.playerGravityControl.gravity_multiplier = value end)
		local playerGravityControlToggle = createMenuToggle(PlayerPower, "Player Gravity Control**local only**", false, function(on) settings.playerGravityControl.isOn = on end)
		

		local realityWarpMenu = menu.list(customizePlayerPower, translate("Customize Reality Warp •     >", "Customize Reality Warp •     >"))
		local triggerDistanceSlider = createMenuSlider(realityWarpMenu, "RW Trigger Distance", "How close before the reality warp is triggered", 0, 500, settings.realityWarp.TriggerDistance, 1, function(value) settings.realityWarp.TriggerDistance = value end)
		local speedBurstMultiplierSlider = createMenuSlider(realityWarpMenu, "RW Speed Burst Multiplier", "Multiplier for the speed burst effect", 0, 50, settings.realityWarp.SpeedBurstMultiplier, 1, function(value) settings.realityWarp.SpeedBurstMultiplier = value end)
		local liftOffHeightSlider = createMenuSlider(realityWarpMenu, "RW Lift Off Height", "Height for the lift off effect", 0, 100, settings.realityWarp.LiftOffHeight, 1, function(value) settings.realityWarp.LiftOffHeight = value end)
		local realityWarpToggle = createMenuToggle(PlayerPower, "Reality Warp", false, function(on) settings.realityWarp.isOn = on end)
		...
		local upwardforceSlider2 = createMenuSlider(customizeNPCPowers, "Gravity Force Threshold • >", "Control npc gravity vertically, up or down!", -1000, 10000, math.floor(downwards_force), 1, function(value) downwards_force = value / 10 end)
		...
		local pushForceMultiplierSlider = createMenuSlider(proximityhoming, "PH Lunge Force Multiplier", "Multiples the Lunge Threshold value by this value", 0, 5000, proximity_push_force_multiplier * 5, 1, function(value) proximity_push_force_multiplier = value / 100 end)
		...
		local pushThresholdSliderBruteF = createMenuSlider(bruteForce, "BruteF Push Threshold (Tenths of a Meter) • >", "Maximum distance between player and other vehicles to trigger the push in tenths of a meter.", 1, 1000, push_threshold * 10, 5, function(value) push_threshold = value / 10 end)
		...
		local explodeDistanceSlider = createMenuSlider(ExplodeOnCollision, "EoC Explosion Distance • >", "How far from the car to the player will count as a collision?", 1, 1000, 75, 1, function(value) settings.explodeOnCollision.explosionDistance = value / 10 end)
		local explodeTypeSlider = createMenuSlider(ExplodeOnCollision, "EoC Explosion Type • >", "P.S 59 is basically a nuke, 72 is EMP, 65 is Kenetic Mine...", 0, 82, 66, 1, function(value) settings.explodeOnCollision.explosionType = value end)
		...
		local audioTypeSlider = createMenuSlider(AudioSpam, "AP Audio Type • >", "", 0, 82, 21, 1, function(value) settings.audioSpam.audioType = value end)
	...
	menu.action(trollyVehicles, translate("Trolling - Attacker Options", "Send Police Car"), {}, "", function()
		local targetPed = PLAYER.GET_PLAYER_PED_SCRIPT_INDEX(pId)
	...
	local enemyVehiclesOpt = menu.list(extrasList, translate("Trolling", "Enemy Vehicles"), {}, "")
