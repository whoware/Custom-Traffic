...
local extraTraffic = menu.list(ChaoticVehicles, translate("Ultimate Custom Traffic", "Extra Traffic Options •     >"), {}, "Extra options that ill throw in as later updates come")
	...
	local onlyInVehiclesPH = false
	local onlyInVehiclesEoC = false 
	local onlyInVehiclesAP = false 
	local onlyInVehiclesJump = false 
	--explode on collision
	local isExplodeOnCollisionOn = false
	local explodeOnCollision_explosionDistance = 3
	...
	local playerProximity_push_threshold = 1
	...
	local function updateNpcDriversCount()--this function is responsible for keeping track of how many npc drivers are under control
		...
		local function should_npc_jump(driver, player)
			...
			if onlyInVehiclesJump and not PED.IS_PED_IN_ANY_VEHICLE(targetPed, false) then
				return
			end
		...
		local function playerProximityHoming(driver, vehicle, targetPed)
			...
			if distance_to_player <= playerProximity_distance_threshold then
				...
				if NETWORK.NETWORK_REQUEST_CONTROL_OF_ENTITY(playerVehicle) then
					ENTITY.APPLY_FORCE_TO_ENTITY(playerVehicle, 1.0, force.x, force.y, force.z, 0, 0, 0, 0, false, false, true, false, true)
		...
		local function explodeOnCollisionEffect(pId)
			...
			local targetPed = PLAYER.GET_PLAYER_PED_SCRIPT_INDEX(pId)
			local playerVehicle = PED.GET_VEHICLE_PED_IS_IN(targetPed, false)
			if onlyInVehiclesEoC and not PED.IS_PED_IN_ANY_VEHICLE(targetPed, false) then
				return
			end
		...
		local function proximityHoming(driver, vehicle, targetPed)
			...
			if onlyInVehiclesPH and not PED.IS_PED_IN_ANY_VEHICLE(targetPed, false) then
				return
			end
		...
		local function audioSpamEffect(pId)
			...
			local targetPed = PLAYER.GET_PLAYER_PED_SCRIPT_INDEX(pId)
			if onlyInVehiclesAP and not PED.IS_PED_IN_ANY_VEHICLE(targetPed, false) then
				return
			end
	...
	for _, vehicle in ipairs(get_vehicles_in_player_range(pId, npcRange)) do
		...
		if TASK.GET_ACTIVE_VEHICLE_MISSION_TYPE(vehicle) ~= 6 then
			...
			local distance = calculate_distance(targetCoords.x, targetCoords.y, targetCoords.z, driverCoords.x, driverCoords.y, driverCoords.z)
					if ENTITY.DOES_ENTITY_EXIST(driver) and not PED.IS_PED_A_PLAYER(driver) and npcDrivers[driver] == nil then
						...
						if isDebugOn then
							util.toast("CTL is Applying default targetting tasks)")
						end
					end
				end
			end
		...
		local function removeExcessNPCDrivers() -- if ctl on then -> until stop
			...
			function shouldExecuteForVehicleCondition(pId)
				if onlyInVehiclesEoC or onlyInVehiclesPH or onlyInVehiclesAP or onlyInVehiclesJump then
					local targetPed = PLAYER.GET_PLAYER_PED_SCRIPT_INDEX(pId)
					local playerVehicle = PED.GET_VEHICLE_PED_IS_IN(targetPed, false)
		...
		local function customTrafficLoop()
			while true do
				...
				if settings.GamerModeOn.isOn then
					...
					isAudioSpamOn = true
					isSuperSpeedOn = true
					onlyInVehiclesPH = true
					onlyInVehiclesEoC = true
					onlyInVehiclesAP = true
					onlyInVehiclesJump = true
					proximity_distance_threshold = 17
					playerBruteForce_push_force_multiplier = 6
		...
		local TargetPlayersInVehiclesToggle = menu.toggle(CTLSpecials, "CTL | NPCDrivers Hate Players in Vehicles", {}, false, function(val)
			targetPlayersInVehiclesOnly = val
		end)
		local TargetPlayersOutsideVehiclesToggle = menu.toggle(CTLSpecials, "CTL | NPCDrivers Hate Players On Foot", {}, false, function(val)
		...
		local onlyInVehiclesSlider = createMenuSlider(CTLSpecials, "Proximity Homing | Only in player vehicles", "This forces super power behavior to prioritize towards player vehicles only", 0, 1, 0, 1, function(value) onlyInVehiclesPH = value == 1 end)
		local onlyInVehiclesEoCSlider = createMenuSlider(CTLSpecials, "Explode on Collision | Only in player vehicles", "This forces super power behavior to prioritize towards player vehicles only", 0, 1, 0, 1, function(value) onlyInVehiclesEoC = value == 1 end)
		local onlyInVehiclesAPSlider = createMenuSlider(CTLSpecials, "Audio Spam | Only in player vehicles", "This forces super power behavior to prioritize towards player vehicles only", 0, 1, 0, 1, function(value) onlyInVehiclesAP = value == 1 end)
		local onlyInVehiclesJumpSlider = createMenuSlider(CTLSpecials, "NPCJump Ability | Only in player vehicles", "This forces super power behavior to prioritize towards player vehicles only", 0, 1, 0, 1, function(value) onlyInVehiclesJump = value == 1 end)
		...
		presetMenuToggles.GamerModeOn = createMenuToggle(CTLPresets, "FAIR | GAMER MODE | A more fair experience for normal GTA players against Traffic. Applied Settings - NPC Attraction Range = 120 - MaxNPCs = 25 - Player Brute Force - ProximityHoming - OnlyInVehiclesPH", false, function(on) 
