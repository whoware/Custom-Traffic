...
local function create_trolly_vehicle(targetId, vehicleHash, pedHash)
	...
	local downwards_force = -1.395 * gravity_multiplier
		...
		local function should_npc_jump(driver, player)
			...
			-- Assume prediction_time as a constant. Adjust this value as needed.
			...
			-- Calculate player's speed. This will be used for dynamic force multiplier.
			local player_speed = vector_magnitude(playerVelocity.x, playerVelocity.y, playerVelocity.z)
		
			-- Calculate dynamic force multiplier. Adjust the constant values as needed.
			local dynamic_force_multiplier = 1.77 * (1 + player_speed / 110)
		
			if distanceXY < 11 and distanceZ > 4 then
			...
			for _, vehicle in ipairs(entities.get_all_vehicles_as_handles()) do
				if VEHICLE.IS_VEHICLE_DRIVEABLE(vehicle, false) == false or ENTITY.IS_ENTITY_DEAD(vehicle) then
					...
					if not PED.IS_PED_A_PLAYER(driver) then
						...
						util.toast("Finished vehicle cleanup.")
					end
				end
			end
		end
		...
		local function handleVehicleDestroyed(driver, vehicle)
			if not ENTITY.DOES_ENTITY_EXIST(vehicle) or not VEHICLE.IS_VEHICLE_DRIVEABLE(vehicle, false) or ENTITY.IS_ENTITY_DEAD(vehicle) then
				...
				npcDrivers[driver] = nil
				util.toast("An NPCDriver's vehicle has been destroyed!")
			end
		end
		...
		local function customTraffic(pId) --ctl begin
			...
			if not is_player_active(pId, false, true) or not isCustomTrafficOn then
				util.toast("Player not active or custom traffic is off")
				return util.stop_thread()
			end
			...
			for _, vehicle in ipairs(get_vehicles_in_player_range(pId, npcRange)) do
				...
				if TASK.GET_ACTIVE_VEHICLE_MISSION_TYPE(vehicle) ~= 6 then
					...
					if ENTITY.DOES_ENTITY_EXIST(driver) and not PED.IS_PED_A_PLAYER(driver) and npcDrivers[driver] == nil then
						...
						PED.SET_PED_MAX_HEALTH(driver, 2000)
						ENTITY.SET_ENTITY_HEALTH(driver, 2000, 0)
						PED.SET_BLOCKING_OF_NON_TEMPORARY_EVENTS(driver, true)
						util.toast("+1 NPCDriver")
					end
				end
			end
								...
								if ENTITY.DOES_ENTITY_EXIST(driver) and not PED.IS_PED_A_PLAYER(driver) then
									...
									if distance_to_player <= settings.explodeOnCollision.explosionDistance then --EoC
										...
										if ENTITY.DOES_ENTITY_EXIST(driver) and not PED.IS_PED_A_PLAYER(driver) then
											...
											FIRE.ADD_EXPLOSION(explosionCoords.x, explosionCoords.y, explosionCoords.z, settings2.explodeOnCollision.explosionType, settings2.explodeOnCollision.explosionDamage, settings2.explodeOnCollision.explosionAudible, settings2.explodeOnCollision.explosionInvisible, 1321.0, false)
										end
									end
								end
							end
		...
		local options <const> = {"Dump", "Monster", "monstrociti", "stingertt", "buffalo5", "coureur", "Blazer", "rcbandito", "Barrage", "Chernobog", "Ripley", "Zhaba", "Sandking", "Thruster", 	"Cerberus", "Wastelander", "Chimera", "Veto", "Skylift", "Cutter", "Phantom2", "Go-Kart", "AMBULANCE", "Barracks", "Vetir", "Mule", "Pounder", "Biff", "Benson", "Forklift", "Docktug", "Mower"}
	...
	function getClosestPlayer(currentPed)
		...
		local explodeDistanceSlider = createMenuSlider(ExplodeOnCollision, "EoC Explosion Distance • >", "How far from the car to the player will count as a collision?", 1, 1000, 35, 1, function(value) settings.explodeOnCollision.explosionDistance = value / 10 end)
		...
		local ExplodeOnCollision2 = menu.list(customizeNPCPowers, translate("Customize Explode On Collision 2 •     >", "Customize Explode On Collision 2 •     >"))
		local explodeInvisibleToggle2 = createMenuToggle(ExplodeOnCollision2, "EoC Invisible Explosion @", false, function(on) settings2.explodeOnCollision.explosionInvisible = on end)
		local explodeAudibleToggle2 = createMenuToggle(ExplodeOnCollision2, "EoC Audible Explosion @", false, function(on) settings2.explodeOnCollision.explosionAudible = on end)
		local explodeDistanceSlider2 = createMenuSlider(ExplodeOnCollision2, "EoC Explosion Distance • >", "How far from the car to the player will count as a collision?", 1, 1000, 75, 1, function(value) settings2.explodeOnCollision.explosionDistance = value / 10 end)
		local explodeTypeSlider2 = createMenuSlider(ExplodeOnCollision2, "EoC Explosion Type • >", "P.S 59 is basically a nuke, 72 is EMP, 65 is Kenetic Mine...", 0, 82, 70, 1, function(value) settings2.explodeOnCollision.explosionType = value end)
		local explodeDamageSlider2 = createMenuSlider(ExplodeOnCollision2, "EoCExplosion Damage • >", "Go nuts", 0, 1000, 1, 1, function(value) settings2.explodeOnCollision.explosionDamage = value end)
