...
--==||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||==--
	...
	local vehiclesToDelete = {} -- Table to store vehicles to delete
	local lastDeletionTime = os.clock()
	

	local function addVehicleToDelete(vehicle)
		table.insert(vehiclesToDelete, vehicle)
	end

	local function deleteVehiclesInTable()
		for _, vehicle in ipairs(vehiclesToDelete) do
			entities.delete(vehicle)
		end
		vehiclesToDelete = {} -- Clear the table after deletion
	end

	--e

	local isDespawnEntitiesOn = true
	...
	local function drawPreset(presetName, presetData)
		...
		local function applyDownwardForce(vehicle)
			if isDownwardsGravityOn and ENTITY.IS_ENTITY_IN_AIR(vehicle) then
		...
		local function deleteDestroyedVehicles()
			...
			for _, vehicle in ipairs(entities.get_all_vehicles_as_handles()) do
				if VEHICLE.IS_VEHICLE_DRIVEABLE(vehicle, false) == false or ENTITY.IS_ENTITY_DEAD(vehicle) then
					...
					if not PED.IS_PED_A_PLAYER(driver) then
						addVehicleToDelete(vehicle)
					end
				end
			end
			if currentTime - lastDeletionTime >= 30 then
				deleteVehiclesInTable()
				lastDeletionTime = currentTime -- Update the time of the last deletion
			end
		end
			...
			if currentTime - lastCheckTime >= 6.0 then
				for _, ped in ipairs(entities.get_all_peds_as_handles()) do
					if ENTITY.IS_ENTITY_DEAD(ped) and not PED.IS_PED_A_PLAYER(ped) then
						if not PED.IS_PED_IN_ANY_VEHICLE(ped, false) then  -- Check if the ped is not in any vehicle
							entities.delete(ped)
						end
					end
				end
		...
		local function customTrafficLoop()
			...
			while true do
				...
				if settings.BumperCarsOn.isOn then
					...
					push_cooldown = math.random(5, 35)
					playerBruteForce_push_cooldown = math.random(0, 35)
					speedMultiplier = 12000
					npcRange = 120
					maxNpcDrivers = 25
...
--==||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||==--
		...
		local despawnEntitiesToggle = createMenuToggle(CTLCustomsXT, "Despawn Dead Peds and Empty Vehicles", true, function(on) isDespawnEntitiesOn = on end)
--==||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||==--
--==||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||              ^ END OF CTL CORE ^               ||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||==--
...
--==||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||==--
		...
		menu.toggle_loop(trollyVehicles, translate("Trolling", "Persistant Random Trolly Vehicles"), {}, "Sends a RANDOM Trolly Vehicle every 5 seconds.", function()
			if not is_player_active(pId, false, true) then
				return util.stop_thread()
			end
