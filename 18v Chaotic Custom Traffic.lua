--==||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||==--
--==||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||     v 18   WHOWARE CUSTOM TRAFFIC & POWERS   v 18      ||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||==--
--==||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||     v 18   WHOWARE CUSTOM TRAFFIC & POWERS   v 18      ||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||==--
--==||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||==--
				...
				if isProximityHomingOn then
					...
					for pId = 0, 31 do
						if is_player_active(pId, false, true) and isCustomTrafficOn then
							local targetPed = PLAYER.GET_PLAYER_PED_SCRIPT_INDEX(pId)
							local playerVehicle = PED.GET_VEHICLE_PED_IS_IN(targetPed, false)
							if playerVehicle ~= 0 or not ignorePlayersOnFoot then
								local isInAir = ENTITY.IS_ENTITY_IN_AIR(playerVehicle)
								if not isInAirOnly or (isInAirOnly and isInAir) or (playerVehicle == 0 and not ignorePlayersOnFoot) then
									Proximity_distance_threshold = 15
									for _, vehicle in ipairs(get_vehicles_in_player_range(pId, 20.0)) do
										local driver = VEHICLE.GET_PED_IN_VEHICLE_SEAT(vehicle, -1, false)
										if ENTITY.DOES_ENTITY_EXIST(driver) and not PED.IS_PED_A_PLAYER(driver) then
...
--==||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||==--
		...
		local CTLSpecials = menu.list(CTLCustoms, translate("Trolling", "CTL Special Settings • >"), {}, "[Alpha - WIP] Apply special super powers that dont exist in native CTL Super powers. These settings give NPCs / Players special specific abilities.")
...
--==||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||==--
		...
		local ignorePlayersOnFootToggle = createMenuToggle(proximityhoming, "Ignore Players on Foot?", false, function(on) ignorePlayersOnFoot = on end)
		local inAirOnlyToggle = createMenuToggle(proximityhoming, "In air only?", false, function(on) isInAirOnly = on end)
