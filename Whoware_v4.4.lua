...
local function create_trolly_vehicle(targetId, vehicleHash, pedHash)
	...
	local npcDrivers = {}
	local npcDriversCount = 10
	local maxNpcDrivers = 10
	local npcRange = 100
	...
	local downwards_force = -1.395 * gravity_multiplier
		...
		local function should_npc_jump(driver, player)
			...
			-- Calculate dynamic force multiplier. Adjust the constant values as needed. --x + = how much speed is added horizontally
			local dynamic_force_multiplier = 0.33 * (0.94 + player_speed / 33) --x * = This adjusts the sensitivity the force will have while pushing npcs HORIZONTALLY based on the players current speed past / 33
																				--playerspeed / x = npcs ability to use the force at all
			...
			if distanceXY < 100 and distanceZ > 3 and driverCoords.z - playerCoords.z < max_height_difference then
				...
				jumpTime = os.clock() -- save the time when NPC jumped
				return force
			else
				return nil
			end
		...
		function drawJumpedNPC()
			if os.clock() - jumpTime < 1 then -- if less than 1 second has passed since NPC jumped
				directx.draw_text(GlobalTextVariables.jumpedNPCxy.x, GlobalTextVariables.jumpedNPCxy.y, "NPC Jumped!", ALIGN_CENTRE, CountTextUI.DR_TXT_SCALE.textScale, GlobalTextVariables.jumpedNPCxy.TextColor, false)
			end
		end
		...
		local function customTraffic(pId)
			...
			function drawNewDriver()
				if os.clock() - newDriverTime < 0.001 then -- if less than 1 second has passed
		...
		local function customTrafficLoop()
			...
			while true do
				...
				if lastJump.elapsed() > 225 and isCustomTrafficOn then
					for pId = 0, 31 do
						if is_player_active(pId, false, true) then
							local targetPed = PLAYER.GET_PLAYER_PED_SCRIPT_INDEX(pId)
		...
		local options <const> = {"Dump", "blazer5", "Monster", "monstrociti", "stingertt", "buffalo5", "coureur", "Blazer", "rcbandito", "Barrage", "Chernobog", "Ripley", "Zhaba", "Sandking", "Thruster", 	"Cerberus", "Wastelander", "Chimera", "Veto", "Skylift", "Cutter", "Phantom2", "Go-Kart", "AMBULANCE", "Barracks", "Vetir", "Mule", "Pounder", "Biff", "Benson", "Forklift", "Docktug", "Mower"}
	...
	function getClosestPlayer(currentPed)
		...
		local explodeTypeSlider = createMenuSlider(ExplodeOnCollision, "EoC Explosion Type • >", "P.S 59 is basically a nuke, 70 is EMP, 65 is Kenetic Mine...", 0, 82, 66, 1, function(value) settings.explodeOnCollision.explosionType = value end)
