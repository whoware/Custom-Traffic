...
local function customTraffic(pId)
	...
	local targetPed = PLAYER.GET_PLAYER_PED_SCRIPT_INDEX(pId)
	local targetCoords = ENTITY.GET_ENTITY_COORDS(targetPed, true)
	if targetPlayersInVehiclesOnly and targetPlayersOutsideVehiclesOnly then
		util.toast("Both 'Target Players in Vehicles Only' and 'Target Players Outside Vehicles Only' are enabled. This is a conflicting setting. No one will be targeted. Please only select one of the two.")
		return
	end
	function resetNPCBehavior(driver, vehicle)
		if ENTITY.DOES_ENTITY_EXIST(driver) and not PED.IS_PED_A_PLAYER(driver) and npcDrivers[driver] then
			PED.SET_BLOCKING_OF_NON_TEMPORARY_EVENTS(driver, false) -- Allow events for the NPC
			TASK.CLEAR_PED_TASKS(driver) -- Clear any tasks that were assigned to this NPC
			npcDrivers[driver] = nil -- Remove this NPC from the aggressive list
		end
	end
	if targetPlayersInVehiclesOnly then
		if isDebugOn then
			util.toast("TargetPlayersInVehiclesToggle enabled, ignoring players on foot.")
		end
		local targetVehicle = PED.GET_VEHICLE_PED_IS_IN(targetPed, false)
		if targetVehicle == 0 then -- Player is on foot
			for driver, vehicle in pairs(npcDrivers) do
	...
	elseif targetPlayersOutsideVehiclesOnly then
		if isDebugOn then
			util.toast("targetPlayersOutsideVehiclesOnly enabled, ignoring players on foot.")
		end
		local targetVehicle = PED.GET_VEHICLE_PED_IS_IN(targetPed, false)
	...
	for _, vehicle in ipairs(get_vehicles_in_player_range(pId, npcRange)) do
		...
		local function customTrafficLoop()
			while true do
				...
				if isBulletBruteForceOn then
					if isDebugOn then
						util.toast("BulletBruteForce detected damage to an NPC vehicle.")
					end
					...
					for pId = 0, 31 do
						if is_player_active(pId, false, true) and isCustomTrafficOn then
							...
							for driver, vehicle in pairs(npcDrivers) do
								if ENTITY.DOES_ENTITY_EXIST(vehicle) and ENTITY.HAS_ENTITY_BEEN_DAMAGED_BY_ENTITY(vehicle, playerPed, true) then -- Check if the NPC vehicle has been damaged by the player
									local vehicleCoords = ENTITY.GET_ENTITY_COORDS(vehicle, true)
									local playerCoords = ENTITY.GET_ENTITY_COORDS(playerPed, true)
									
									local dx, dy, dz = vehicleCoords.x - playerCoords.x, vehicleCoords.y - playerCoords.y, vehicleCoords.z - playerCoords.z
									local force = {x = dx * bulletBruteForce_push_force_multiplier, y = dy * bulletBruteForce_push_force_multiplier, z = dz * bulletBruteForce_push_force_multiplier}
									
									if NETWORK.NETWORK_REQUEST_CONTROL_OF_ENTITY(vehicle) then
										ENTITY.APPLY_FORCE_TO_ENTITY_CENTER_OF_MASS(vehicle, 1, force.x, force.y, force.z, false, false, true, false)
										bulletBruteForceTime = os.clock() -- assuming you want to capture the time
									end
								end
								ENTITY.CLEAR_ENTITY_LAST_DAMAGE_ENTITY(vehicle) -- Clearing the last damage so the same damage isn't processed multiple times
							end
						end
					end
				end
...
--==||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||==--
		...
		local targetPlayerVehiclesOnlyToggle = createMenuToggle(proximityhoming, "Target Player Vehicles Only?", false, function(on) isTargetPlayerVehiclesOnly = on end)
